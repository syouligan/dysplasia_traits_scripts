...
/share/ScratchGeneral/scoyou/local/bin/MAGMA/magma --annotate window=1 --snp-loc $workDir/$inFile --gene-loc $geneLoc --out $workDir/$trait

# Calculates genewise enrichment using mean method
/share/ScratchGeneral/scoyou/local/bin/MAGMA/magma --bfile $genome --gene-annot $workDir/*.genes.annot --gene-model snp-wise=mean --seed 1 --pval $workDir/*_MAGMA.input.txt use=1,4 ncol=5 --out $workDir/$trait'.mean'

# Calculates genewise enrichment using multi method
/share/ScratchGeneral/scoyou/local/bin/MAGMA/magma --bfile $genome --gene-annot $workDir/*.genes.annot --gene-model snp-wise=multi --seed 1 --pval $workDir/*_MAGMA.input.txt use=1,4 ncol=5 --out $workDir/$trait'.multi'
