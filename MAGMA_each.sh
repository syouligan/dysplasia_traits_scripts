...
# Run MAGMA on all folders in outPath
# usage: writes to sample input directory
# ./MAGMA_each.sh (outPath) (inFile) (geneLoc) (trait) (genome) (geneList)
...
outPath=$1 # directory for output based on geneset
...
/share/ScratchGeneral/scoyou/local/bin/MAGMA/magma --annotate window=1 --snp-loc $inFile --gene-loc $geneLoc --out $outPath/$trait

# Calculates genewise enrichment using mean method
/share/ScratchGeneral/scoyou/local/bin/MAGMA/magma --bfile $genome --gene-annot $outPath/*.genes.annot --gene-model snp-wise=mean --seed 1 --pval $inFile use=1,4 ncol=5 --out $outPath/$trait'.mean'

# Calculates genewise enrichment using multi method
/share/ScratchGeneral/scoyou/local/bin/MAGMA/magma --bfile $genome --gene-annot $outPath/*.genes.annot --gene-model snp-wise=multi --seed 1 --pval $inFile use=1,4 ncol=5 --out $outPath/$trait'.multi'

# Calculates geneset enrichment using mean method
/share/ScratchGeneral/scoyou/local/bin/MAGMA/magma --gene-results $outPath/*mean.genes.raw --set-annot $geneList --out $outPath/$trait'.mean'

# Calculates geneset enrichment using multi method
/share/ScratchGeneral/scoyou/local/bin/MAGMA/magma --gene-results $outPath/*multi.genes.raw --set-annot $geneList --out $outPath/$trait'.multi'
